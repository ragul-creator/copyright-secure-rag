...
def chat(question: str, top_k: int = 4):
    request_id=str(uuid.uuid4())
    contexts=retriever.search(question,top_k)
    answer=generator.generate(question,contexts)
    policy,signals=guard.evaluate(request_id,answer,contexts)
    notes=[]
    ...
    if decision=="REWRITE":
        ...
        p2,s2=guard.evaluate(request_id,retry,contexts)
