        ...
        if store:
            ...
            for r in candidates:
                ...
                if src and _can_use(tenant_id, src):
                    approved.append({**r,"source_name":src["name"],"status":src["status"],
                        "rag_allowed":src["rag_allowed"],"generation_allowed":src["generation_allowed"],
                        "expires_at":src.get("expires_at"),"attribution_required":src["attribution_required"],
                        "quote_word_limit":src["quote_word_limit"],"license_spdx":src["license_spdx"]})
