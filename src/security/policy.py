...
def decide(signals: dict, contexts: list[dict]):
    local=local_decision(signals,contexts)
    if not settings.opa_url:
        return local
    payload={"input":{"signals":signals,"contexts":[{
        "source_id":c["source_id"],"quote_word_limit":c.get("quote_word_limit",settings.max_exact_words),
        "attribution_required":bool(c.get("attribution_required"))} for c in contexts]}}
    try:
