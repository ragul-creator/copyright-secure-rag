...
def api(method, path, **kwargs):
    ...
    try:
        r = requests.request(method, API + path, timeout=45, **kwargs)
...
chat_tab, kb_tab, security_tab = st.tabs(
    ["Support chat", "Knowledge & rights", "Security operations"]
)
            ...
            with st.chat_message("assistant"):
                ...
                if data.get("citations"):
                    ...
                    for c in data["citations"]:
                        st.write(
                            f"- {c['source_name']} ({c['document_id']}) — retrieval {c['score']}"
                        )
...
with kb_tab:
    ...
    with right:
        st.subheader("Ingest support document")
        did = st.text_input("Document ID", "DOC-001")
        uploaded = st.file_uploader(
            "Upload TXT, Markdown, HTML or PDF",
            type=["txt", "md", "markdown", "html", "htm", "pdf"],
        )
        if uploaded is not None and st.button("Upload and ingest"):
            files = {
                "file": (
                    uploaded.name,
                    uploaded.getvalue(),
            ...
            st.json(
                api(
                    "POST",
                    "/ingest/file",
                    data={"source_id": sid, "document_id": did},
                    files=files,
                )
                or {}
            )
        ...
        with st.expander("Or paste text directly"):
            txt = st.text_area("Document text", height=160)
            if st.button("Ingest pasted text"):
                st.json(
                    api(
                        "POST",
                        "/ingest",
                        json={"source_id": sid, "document_id": did, "text": txt},
                    )
                    or {}
                )
    ...
    with ev1:
        tool = st.selectbox("Scanner/tool", ["scancode", "ort", "manual", "other"])
    with ev2:
        verdict = st.selectbox("Verdict", ["approved", "review", "rejected"])
    with ev3:
        artifact = st.text_input(
            "Artifact URI", "file://compliance-results/manifest.json"
        )
    evidence_text = st.text_area(
        "Evidence/details JSON", value='{"note":"reviewed"}', height=80
    )
    if st.button("Attach compliance evidence"):
        try:
            details = json.loads(evidence_text or "{}")
        except json.JSONDecodeError:
            st.error("Evidence/details must be valid JSON")
            details = None
...
with security_tab:
    ...
    if overview:
        ...
        st.metric(
            "Audit chain",
            "VALID" if overview.get("audit_chain_valid") else "INVALID",
        )

    readiness = api("GET", "/readyz")
    if readiness:
        st.caption(
            f"Runtime: {readiness.get('status')} • database={readiness.get('database')} • vector={readiness.get('vector_backend')}"
        )
